...
const Timer = (props: Props) => {
  ...
  return (
    <div>
      {showTimer ? (
        <div
          className="flex items-center bg-dark-fill-3 p-1.5 cursor-pointer rounded hover:bg-dark-fill-2 gap-2"
          onClick={handleClockClick}
        >
          <div>{formatTime(time)}</div>
          <FiRefreshCcw onClick={clearClockTimer} />
        </div>
      ) : (
        <div
