...
const Topbar = (props: Props) => {
  const { problemPage = false, DBProblems } = props;
  ...
  const handleProblemChange = (isForward: boolean) => {
    // const { order } = DBProblems[router.query.id as string];
    const problemIdx = DBProblems.findIndex(
      (p) => p.id === (router.query.id as string)
    );
    const problem = problemIdx > 0 ? DBProblems[problemIdx] : null;
    if (!problem) return;

    const order = problem.order;
    const direction = isForward ? 1 : -1;
    const nextProblemOrder = order + direction;
    const nextProblemKey = DBProblems[problemIdx + direction];
